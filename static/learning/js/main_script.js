...
$(".lmblock").click(function(){
	if(counter % 2 == 0){
		...
		setTimeout(function(){
			$("#menuHand li:nth-child(6) a").css({"margin-top": "0"})
		}, 500);
		setTimeout(function(){
			$("#menuHand li:nth-child(7) a").css({"margin-top": "0"})
		}, 500);
		setTimeout(function(){
			$("#menuHand li:nth-child(8) a").css({"margin-top": "0"})
		}, 500);
		$("#menuHand li a").css({"display": "block"});
		$("#menuHand").css({"display": "flex"});
		$("#menuHand").css({"height": "100%"});
		$("#lm1, #lm2, #lm3").css({"background": "#3498DB"});
		$(".lm-title").css({"color": "#3498DB"});
